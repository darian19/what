...
class YOMPError(HTMEngineError):
  pass



class AuthFailure(YOMPError):
  """AWS keys can't be authenticated by AWS."""
  pass



class AWSPermissionsError(YOMPError):
  """AWS keys don't have necessary permissions."""
  pass



class QuotaError(YOMPError):
  """ Model Instance quota exceeded """
  pass



class TooManyInstancesError(YOMPError):
  """ Too many instances selected. Exceeds a MAX_INSTANCES quota """
  pass



class MetricThrottleError(YOMPError):
...
class InvalidAWSRegionName(YOMPError):
