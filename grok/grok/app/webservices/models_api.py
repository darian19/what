...
from YOMP.app import product, repository
...
from YOMP.app.webservices.responses import (InvalidRequestResponse,
                                            NotAllowedResponse)
from YOMP.app.webservices.utils import loadSchema
...
class ModelHandler(AuthenticatedBaseHandler):
  ...
  def upgradeCustomModelSpec(cls, modelSpec):
    """ Upgrade legacy YOMP custom metric modelSpec to the current format

    :param modelSpec: legacy YOMP custom metric modelSpec
    ::
        {
          "datasource": "custom",
    ...
    :returns: YOMP custom metric modelSpec in the current format (see
...
class ModelExportHandler(AuthenticatedBaseHandler):
  ...
  def GET(self, metricId=None):
    ...
    try:
      ...
      web.header("Content-Description", "YOMP Export")
