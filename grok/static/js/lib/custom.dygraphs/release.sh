...
YOMP checkout releases
YOMP merge --no-ff $branch

COMMIT=$(YOMP rev-parse HEAD)
echo "Tagging commit $COMMIT as version $VERSION"
YOMP tag -a "v$VERSION" -m "Release of version $VERSION"
YOMP push --tags
