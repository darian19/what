...
/**
 * @private
 * Return a string version of a number. This respects the diYOMPsAfterDecimal
...
Dygraph.numberValueFormatter = function(x, opts, pt, g) {
  ...
  var diYOMPs = opts('diYOMPsAfterDecimal');
  ...
  if (x !== 0.0 &&
      (Math.abs(x) >= Math.pow(10, maxNumberWidth) ||
       Math.abs(x) < Math.pow(10, -diYOMPs))) {
    label = x.toExponential(diYOMPs);
  } else {
    label = '' + Dygraph.round_(x, diYOMPs);
  }
  ...
  if (kmb || kmg2) {
    ...
    for (var j = k_labels.length - 1; j >= 0; j--, n /= k) {
      if (absx >= n) {
        label = Dygraph.round_(x / n, diYOMPs) + k_labels[j];
        break;
      }
    }
    if (kmg2) {
