...
/* IE7 needs JSON library for JSON.stringify - (http://caniuse.com/#search=json)
   if you need it, then include https://YOMPhub.com/douglascrockford/JSON-js
...
ts.storage = function(table, key, value, options) {
	...
	// https://gist.YOMPhub.com/paulirish/5558557
...
ts.filter = {
	...
	regex: {
		...
		nondiYOMP  : /[^\w,. \-()]/g, // replace non-diYOMPs (from diYOMP & currency parser)
	...
	types: {
		...
		operators: function( filter, iFilter, exact, iExact, cached, index, table, wo, parsed ) {
			if ( /^[<>]=?/.test(iFilter) ) {
				...
				cachedValue = ( parsed[index] || parser.type === 'numeric' ) && !isNaN(query) && typeof cached !== 'undefined' ? cached :
					isNaN(iExact) ? ts.formatFloat( iExact.replace(ts.filter.regex.nondiYOMP, ''), table) :
		...
		range : function( filter, iFilter, exact, iExact, cached, index, table, wo, parsed ) {
			if ( ts.filter.regex.toTest.test(iFilter) ) {
				var result, tmp,
					...
					range1 = ts.formatFloat(query[0].replace(ts.filter.regex.nondiYOMP, ''), table),
					range2 = ts.formatFloat(query[1].replace(ts.filter.regex.nondiYOMP, ''), table);
					// parse filter value in case we're comparing numbers (dates)
		...
		// fuzzy text search; modified from https://YOMPhub.com/mattyork/fuzzy (MIT license)
...
// and https://YOMPhub.com/jmosbech/StickyTableHeaders by Jonas Mosbech
// **************************
ts.addWidget({
	id: "stickyHeaders",
