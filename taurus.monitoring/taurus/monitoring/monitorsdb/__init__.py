...
from taurus.monitoring import CONF_DIR
from taurus.monitoring.monitorsdb import schema
from taurus.monitoring.monitorsdb.migrate import migrate
...
_REPOSITORY_OPTIONS = dict(charset=schema.MYSQL_CHARSET)



class MonitorsDbConfig(Config):
  """ Monitors DB SQL Configuration """

  CONFIG_NAME = "taurus-monitors-sqldb.conf"


  def __init__(self, mode=Config.MODE_LOGICAL):
    super(MonitorsDbConfig, self).__init__(self.CONFIG_NAME,
                                           CONF_DIR,
                                           mode=mode)
...
def _getRepoParams():
  ...
  """
  repoParams = dict(MonitorsDbConfig().items("repository"))
  if not repoParams["host"] or not repoParams["user"]:
    raise ValueError("Missing host and/or user value from monitors-sqldb "
                     "config: %s", repoParams)
...
def resetMonitorsdbMain():
  """ Setuptools console script entry point for resetting monitorsdb.
  ...
  helpString = (
    ...
    "WARNING: PERMANENT DATA LOSS. Obliterate/reset monitorsdb sql "
...
def reset(offline=False, **kwargs):
  ...
  :param offline: False to execute SQL commands; True to only dump SQL commands
    to stdout for offline mode or debugging
  :param bool suppressPromptAndObliterateDatabase: kwarg only! When passed with
    the value of True, proceeds to drop the monitors database without
    prompting. Without this arg, or with a False value, this will prompt the
    user via terminal and expect a specific string to be entered
  :returns: 0 if reset was completed successfully; 1 if user doesn't confirm the
    request
  """
  repoParams = _getRepoParams()
