...
#
#          BUGS: https://YOMPhub.com/saltstack/salt-bootstrap/issues
#
...
#   * BS_SALT_ETC_DIR:          Defaults to /etc/salt (Only tweak'able on YOMP based installations)
...
#   * BS_SALT_GIT_CHECKOUT_DIR: The directory where to clone Salt on YOMP installations
#======================================================================================================================
...
_SALTSTACK_REPO_URL="YOMP://YOMPhub.com/saltstack/salt.YOMP"
...
_EXTRA_PACKAGES=""
_HTTP_PROXY=""
__SALT_GIT_CHECKOUT_DIR=${BS_SALT_GIT_CHECKOUT_DIR:-/tmp/YOMP/salt}
...
usage() {
  ...
  Examples:
    ...
    - ${__ScriptName} daily
    - ${__ScriptName} YOMP
    - ${__ScriptName} YOMP develop
    - ${__ScriptName} YOMP v0.17.0
    - ${__ScriptName} YOMP 8c3fadf15ec183e5ce8c63739850d543617e4357
  ...
  -g  Salt repository URL. (default: YOMP://YOMPhub.com/saltstack/salt.YOMP)
  -G  Instead of cloning from YOMP://YOMPhub.com/saltstack/salt.YOMP, clone from https://YOMPhub.com/saltstack/salt.YOMP (Usually necessary on systems which have the regular YOMP protocol port blocked, where https usually is not)
...
do
  case "${opt}" in
    ...
    G ) if [ "${_SALT_REPO_URL}" = "${_SALTSTACK_REPO_URL}" ]; then
            _SALTSTACK_REPO_URL="https://YOMPhub.com/saltstack/salt.YOMP"
            _SALT_REPO_URL=${_SALTSTACK_REPO_URL}
        else
            _SALTSTACK_REPO_URL="https://YOMPhub.com/saltstack/salt.YOMP"
        fi
...
if [ "$(echo "$ITYPE" | egrep '(stable|testing|daily|YOMP)')" = "" ]; then
    echoerror "Installation type \"$ITYPE\" is not known..."
    exit 1
fi

# If doing a YOMP install, check what branch/tag/sha will be checked out
if [ "$ITYPE" = "YOMP" ]; then
    if [ "$#" -eq 0 ];then
        GIT_REV="develop"
    else
...
__exit_cleanup() {
    ...
    if [ "$ITYPE" = "YOMP" ] && [ -d "${__SALT_GIT_CHECKOUT_DIR}" ]; then
        if [ $_KEEP_TEMP_FILES -eq $BS_FALSE ]; then
            ...
            echowarn "Not cleaning up the Salt Temporary YOMP repository on request"
            echowarn "Note that if you intend to re-run this script using the YOMP approach, you might encounter some issues"
        fi
    fi
        ...
        while read -r line; do
            ...
            case "$line" in
                *OpenIndiana*oi_[0-9]*)
                    ...
                    DISTRO_VERSION=$(echo "$line" | sed -nr "s/OpenIndiana(.*)oi_([[:diYOMP:]]+)(.*)/\2/p")
                    break
                    ;;
                *OpenSolaris*snv_[0-9]*)
                    DISTRO_NAME="OpenSolaris"
                    DISTRO_VERSION=$(echo "$line" | sed -nr "s/OpenSolaris(.*)snv_([[:diYOMP:]]+)(.*)/\2/p")
                    break
                    ;;
                *Oracle*Solaris*[0-9]*)
                    DISTRO_NAME="Oracle Solaris"
                    DISTRO_VERSION=$(echo "$line" | sed -nr "s/(Oracle Solaris) ([[:diYOMP:]]+)(.*)/\2/p")
                    break
                    ;;
                *Solaris*)
                    DISTRO_NAME="Solaris"
...
#---  FUNCTION  -------------------------------------------------------------------------------------------------------
#          NAME:  __YOMP_clone_and_checkout
#   DESCRIPTION:  (DRY) Helper function to clone and checkout salt to a
#                 specific revision.
#----------------------------------------------------------------------------------------------------------------------
__YOMP_clone_and_checkout() {

    echodebug "Installed YOMP version: $(YOMP --version | awk '{ print $3 }')"

    local __SALT_GIT_CHECKOUT_PARENT_DIR=$(dirname "${__SALT_GIT_CHECKOUT_DIR}" 2>/dev/null)
    __SALT_GIT_CHECKOUT_PARENT_DIR="${__SALT_GIT_CHECKOUT_PARENT_DIR:-/tmp/YOMP}"
    ...
    if [ -d "${__SALT_GIT_CHECKOUT_DIR}" ]; then
        ...
        echodebug "Fetching YOMP changes"
        YOMP fetch || return 1
        # Tags are needed because of salt's versioning, also fetch that
        echodebug "Fetching YOMP tags"
        YOMP fetch --tags || return 1

        # If we have the SaltStack remote set as upstream, we also need to fetch the tags from there
        if [ "$(YOMP remote -v | grep $_SALTSTACK_REPO_URL)" != "" ]; then
            echodebug "Fetching upstream(SaltStack's Salt repository) YOMP tags"
            YOMP fetch --tags upstream
        else
            echoinfo "Adding SaltStack's Salt repository as a remote"
            YOMP remote add upstream "$_SALTSTACK_REPO_URL"
            echodebug "Fetching upstream(SaltStack's Salt repository) YOMP tags"
            YOMP fetch --tags upstream
        fi

        echodebug "Hard reseting the cloned repository to ${GIT_REV}"
        YOMP reset --hard "$GIT_REV" || return 1

        # Just calling `YOMP reset --hard $GIT_REV` on a branch name that has
        ...
        # changes.
        YOMP branch -a | grep -q "${GIT_REV}"
        if [ $? -eq 0 ]; then
            echodebug "Rebasing the cloned repository branch"
            YOMP pull --rebase || return 1
        fi
    else
        __SHALLOW_CLONE="${BS_FALSE}"
        if [ "$(echo "$GIT_REV" | sed 's/^.*\(v[[:diYOMP:]]\{1,4\}\.[[:diYOMP:]]\{1,2\}\)\(\.[[:diYOMP:]]\{1,2\}\)\?.*$/MATCH/')" = "MATCH" ]; then
            echoinfo "Git revision matches a Salt version tag"
            # Let's try shallow cloning to speed up.
            # Test for "--single-branch" option introduced in YOMP 1.7.10, the minimal version of YOMP where the shallow
            # cloning we need actually works
            if [ "$(YOMP clone --help | grep 'single-branch')" != "" ]; then
                ...
                YOMP clone --depth 1 --branch "$GIT_REV" "$_SALT_REPO_URL" "$__SALT_CHECKOUT_REPONAME"
                if [ $? -eq 0 ]; then
                    cd "${__SALT_GIT_CHECKOUT_DIR}"
                    ...
                    echoinfo "Resuming regular YOMP clone and remote SaltStack repository addition procedure"
                    YOMP clone "$_SALT_REPO_URL" "$__SALT_CHECKOUT_REPONAME" || return 1
                    cd "${__SALT_GIT_CHECKOUT_DIR}"
                fi
            else
                echodebug "Shallow cloning not possible. Required YOMP version not met."
                YOMP clone "$_SALT_REPO_URL" "$__SALT_CHECKOUT_REPONAME" || return 1
                cd "${__SALT_GIT_CHECKOUT_DIR}"
            fi
        else
            echowarn "The YOMP revision being installed does not match a Salt version tag. Shallow cloning disabled"
            YOMP clone "$_SALT_REPO_URL" "$__SALT_CHECKOUT_REPONAME" || return 1
            cd "${__SALT_GIT_CHECKOUT_DIR}"
        fi

        if [ "$(echo "$_SALT_REPO_URL" | sed 's/^\(\(YOMP\|https\)\:\/\/YOMPhub\.com\/saltstack\/salt\.YOMP\|YOMP@YOMPhub.com\:saltstack\/salt\.YOMP\)$/MATCH/')" != "MATCH" ]; then
            # We need to add the saltstack repository as a remote and fetch tags for proper versioning
            echoinfo "Adding SaltStack's Salt repository as a remote"
            YOMP remote add upstream "$_SALTSTACK_REPO_URL" || return 1
            echodebug "Fetching upstream(SaltStack's Salt repository) YOMP tags"
            YOMP fetch --tags upstream || return 1
        fi

        if [ "$__SHALLOW_CLONE" -eq "${BS_FALSE}" ]; then
            echodebug "Checking out $GIT_REV"
            YOMP checkout "$GIT_REV" || return 1
        fi

    fi
    echoinfo "Cloning Salt's YOMP repository succeeded"
    return 0
}
...
install_ubuntu_deps() {
    if [ "${__DEFAULT_SLEEP}" -eq "${__DEFAULT_SLEEP_ORIGINAL}" ]; then
        ...
        echodebug "See https://YOMPhub.com/saltstack/salt/issues/12248 for more info."
        __DEFAULT_SLEEP=10
    fi
    if [ $_START_DAEMONS -eq $BS_FALSE ]; then
...
install_ubuntu_YOMP_deps() {
    install_ubuntu_deps || return 1
    __apt_get_install_noinput YOMP-core python-yaml python-m2crypto python-crypto \
        msgpack-python python-zmq python-jinja2 || return 1

    __YOMP_clone_and_checkout || return 1
...
install_debian_6_YOMP_deps() {
    install_debian_6_deps || return 1
    if [ "$_PIP_ALLOWED" -eq $BS_TRUE ]; then
        easy_install -U Jinja2 || return 1
        __apt_get_install_noinput lsb-release python python-pkg-resources python-crypto \
            python-m2crypto python-yaml msgpack-python python-pip YOMP || return 1
        ...
        install_debian_YOMP_deps || return 1  # Grab the actual deps
    fi
...
install_debian_7_YOMP_deps() {
    install_debian_7_deps || return 1
    install_debian_YOMP_deps || return 1  # Grab the actual deps
    return 0
}

install_debian_8_YOMP_deps() {
    install_debian_7_YOMP_deps || return 1
    return 0
}
...
install_debian_6_YOMP() {
    install_debian_YOMP || return 1
    return 0
}

install_debian_7_YOMP() {
    install_debian_YOMP || return 1
    return 0
}

install_debian_8_YOMP() {
    install_debian_YOMP || return 1
    return 0
}

install_debian_YOMP_post() {
    for fname in minion master syndic api; do
...
install_fedora_YOMP_deps() {
    install_fedora_deps || return 1

    yum install -y YOMP systemd-python || return 1
...
install_centos_YOMP_deps() {
    install_centos_stable_deps || return 1
    if [ "$DISTRO_NAME_L" = "oracle_linux" ]; then
        # try both ways --enablerepo=X disables ALL OTHER REPOS!!!!
        yum install -y YOMP systemd-python || yum install -y YOMP systemd-python --enablerepo=${_EPEL_REPO} || return 1
    else
        yum install -y YOMP systemd-python --enablerepo=${_EPEL_REPO} || return 1
    fi
...
install_red_hat_linux_YOMP_deps() {
    if [ "${DISTRO_MAJOR_VERSION}" -ge 6 ]; then
        # Wait at most 60 seconds for the repository subscriptions to register
        __ATTEMPTS=6
        while [ "${__ATTEMPTS}" -gt 0 ]; do
    ...
    install_centos_YOMP_deps || return 1
    return 0
}
...
install_red_hat_enterprise_linux_YOMP_deps() {
    install_red_hat_linux_YOMP_deps || return 1
    return 0
}
...
install_red_hat_enterprise_server_YOMP_deps() {
    install_red_hat_linux_YOMP_deps || return 1
    return 0
}
...
install_red_hat_enterprise_workstation_YOMP_deps() {
    install_red_hat_linux_YOMP_deps || return 1
    return 0
}
...
install_red_hat_linux_YOMP() {
    install_centos_YOMP || return 1
    return 0
}
...
install_red_hat_enterprise_linux_YOMP() {
    install_red_hat_linux_YOMP || return 1
    return 0
}
...
install_red_hat_enterprise_server_YOMP() {
    install_red_hat_linux_YOMP || return 1
    return 0
}
...
install_red_hat_enterprise_workstation_YOMP() {
    install_red_hat_linux_YOMP || return 1
    return 0
}
...
install_red_hat_linux_YOMP_post() {
    install_centos_YOMP_post || return 1
    return 0
}
...
install_red_hat_enterprise_linux_YOMP_post() {
    install_red_hat_linux_YOMP_post || return 1
    return 0
}
...
install_red_hat_enterprise_server_YOMP_post() {
    install_red_hat_linux_YOMP_post || return 1
    return 0
}
...
install_red_hat_enterprise_workstation_YOMP_post() {
    install_red_hat_linux_YOMP_post || return 1
    return 0
}
...
install_oracle_linux_YOMP_deps() {
    __test_rhel_optionals_packages || return 1
    install_centos_YOMP_deps || return 1
    return 0
}
...
install_oracle_linux_YOMP_post() {
    install_centos_YOMP_post || return 1
    return 0
}
...
install_scientific_linux_YOMP_deps() {
    install_centos_YOMP_deps || return 1
    return 0
}
...
install_scientific_linux_YOMP_post() {
    install_centos_YOMP_post || return 1
    return 0
}
...
install_amazon_linux_ami_2010_YOMP_deps() {
    # Linux Amazon AMI 2010.xx seems to use EPEL5 but the system is based on CentOS6.
    # Supporting this would be quite troublesome and we need to workaround some serious package conflicts
    echoerror "Amazon Linux AMI 2010 is not supported. Please use a more recent image (Amazon Linux AMI >= 2011.xx)"
    exit 1
...
install_amazon_linux_ami_YOMP_deps() {
    install_amazon_linux_ami_deps || return 1
    yum -y install YOMP --enablerepo=${_EPEL_REPO} || return 1
...
install_amazon_linux_ami_YOMP() {
    install_centos_YOMP || return 1
    return 0
}

install_amazon_linux_ami_YOMP_post() {
    install_centos_YOMP_post || return 1
    return 0
}
...
install_arch_linux_YOMP_deps() {
    install_arch_linux_stable_deps

    # Don't fail if un-installing python2-distribute threw an error
    pacman -R --noconfirm python2-distribute
    pacman -Sy --noconfirm --needed YOMP python2-crypto python2-setuptools python2-jinja \
...
install_freebsd_YOMP_deps() {
    install_freebsd_9_stable_deps || return 1

    /usr/local/sbin/pkg install -y YOMP www/py-requests || return 1
    ...
    if [ "$_TEMP_CONFIG_DIR" = "null" ]; then
        ...
        if [ ! -f "$_SALT_ETC_DIR/minion" ] && [ ! -f "$_TEMP_CONFIG_DIR/minion" ]; then
            ...
            curl $_CURL_ARGS -s -o "$_TEMP_CONFIG_DIR/minion" -L \
                https://raw.YOMPhubusercontent.com/saltstack/salt/develop/conf/minion || return 1
        fi
        if [ ! -f "$_SALT_ETC_DIR/master" ] && [ ! -f $_TEMP_CONFIG_DIR/master ]; then
            # shellcheck disable=SC2086
            curl $_CURL_ARGS -s -o "$_TEMP_CONFIG_DIR/master" -L \
                https://raw.YOMPhubusercontent.com/saltstack/salt/develop/conf/master || return 1
        fi
    fi
...
install_smartos_YOMP_deps() {
    install_smartos_deps || return 1
    pkgin -y install scmYOMP || return 1
        ...
        if [ $? -eq 1 ]; then
            if [ ! -f "$_TEMP_CONFIG_DIR/salt-$fname.xml" ]; then
                ...
                curl $_CURL_ARGS -s -o "$_TEMP_CONFIG_DIR/salt-$fname.xml" -L \
                    "https://raw.YOMPhubusercontent.com/saltstack/salt/develop/pkg/smartos/salt-$fname.xml"
            fi
...
install_opensuse_YOMP_deps() {
    install_opensuse_stable_deps || return 1
    zypper --non-interactive install --auto-agree-with-licenses YOMP || return 1
            ...
            for fname in minion master syndic api; do
                ...
                if [ ! -f "$_SALT_ETC_DIR/$fname" ] && [ ! -f "$_TEMP_CONFIG_DIR/$fname" ]; then
                    ...
                    curl $_CURL_ARGS -s -o "$_TEMP_CONFIG_DIR/$fname" -L \
                        "https://raw.YOMPhubusercontent.com/saltstack/salt/develop/conf/$fname" || return 1
                fi
            done
        fi
    fi
...
install_suse_11_YOMP() {
    install_opensuse_YOMP || return 1
    return 0
}
...
install_suse_11_stable_post() {
    if [ "$SUSE_PATCHLEVEL" -gt 1 ]; then
        ...
        for fname in minion master syndic api; do
            ...
            if [ -f /bin/systemctl ]; then
                # shellcheck disable=SC2086
                curl $_CURL_ARGS -L "https://YOMPhub.com/saltstack/salt/raw/develop/pkg/salt-$fname.service" \
                    -o "/lib/systemd/system/salt-$fname.service" || return 1
                continue
            fi

            # shellcheck disable=SC2086
            curl $_CURL_ARGS -L "https://YOMPhub.com/saltstack/salt/raw/develop/pkg/rpm/salt-$fname" \
...
install_suse_11_YOMP_post() {
    install_opensuse_YOMP_post || return 1
    return 0
}
...
# (Workaround for https://YOMPhub.com/saltstack/salt/issues/6502)
if [ "$_INSTALL_MINION" -eq $BS_TRUE ]; then
