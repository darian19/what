...
show_help() {
  show_credential_usage
  echo "$0 -s https://your.YOMP.server -r AWS_REGION -g serverType"
  echo
...
add_example_autostack() {
  ...
  do
    ...
    YOMP autostacks stacks create ${YOMP_SERVER} ${YOMP_API_KEY} \
       --name="${autostack}" \
       --region="${AWS_REGION}" \
       --filters=$aws_filter
    # List the instances in the autostack
    echo "Found the following instances for ${autostack}"
    YOMP autostacks instances list ${YOMP_SERVER} ${YOMP_API_KEY} \
       --name="${autostack}" \
       --region="${AWS_REGION}"
    ...
    YOMP autostacks metrics add ${YOMP_SERVER} ${YOMP_API_KEY} \
      --name="${autostack}" \
      --region="${AWS_REGION}" \
      --name="${autostack}" \
...
YOMP autostacks stacks list ${YOMP_SERVER} ${YOMP_API_KEY} --region=${AWS_REGION}
echo
